...
contract ICO is Ownable {
  ...
  uint public teamPercent = 20;
  uint public adviserAndBountiesPercent = 10;
  ...
  function ICO(address _multisig, address _tokenAddress, address _team, address _adviserAndBounties, uint _rate, uint _start, uint _period) {
    multisig = _multisig;
    team = _team;
    ...
    refundAllowed = false;
    crowdsaleHalted= false;
    //softcap = 1000000000000000000000;
  }
  
  modifier saleIsOn() {
    require(now > start && now < start + period * 1 days && !crowdsaleHalted);
    _;
  }
  ...
  function haltCrowdasle () onlyOwner {
    crowdsaleHalted = true;
  }
  function unHaltCrowdasle() onlyOwner{
    require(!crowdsaleHalted);
    crowdsaleHalted = false ;
  ...
  function  refund() public {
    ...
    //require(refundAllowed && now > start + period * 1 days);
  ...
  function finishMinting() public onlyOwner {
    ...
    uint teamTokens = issuedTokenSupply.mul(teamPercent).div(70);
    uint adviserAndBountiesTokens = issuedTokenSupply.mul(adviserAndBountiesPercent).div(70);
    token.mint(adviserAndBounties, adviserAndBountiesTokens);
